    ...
    render() {
        ...
        return (
            <KeyboardAvoidingView style={styles.container} behavior="padding" enabled>
                <Text style={{ fontSize: 20, marginBottom: 20 }}>{this.props.User ? `Hi , ${name}` : `Hi`}</Text>
                <Text style={{ fontSize: 20, marginBottom: 40 }}>Please enter your new Deck title</Text>
                <TextInput
                    style={input.field}
                    ...
                    onPress={() => {
                        this.props.dispatch(addNewDeck(this.state.DeckTitle)).then(() => {
                            this.props.navigation.navigate('deck',{
                                singleDeck : {title: this.state.DeckTitle}
                            })
                        })
